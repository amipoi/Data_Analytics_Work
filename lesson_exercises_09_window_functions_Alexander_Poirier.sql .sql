...
SELECT 
	...
	COUNT(length) OVER () AS conut_length_for_all_films,
	MIN(length) OVER () AS min_length_for_all_films,
	MAX(length) OVER () AS max_length_for_all_films,
	SUM(length) OVER () AS sum_length_for_all_films
FROM film;
...
*/
SELECT 
	ROW_NUMBER() OVER (PARTITION BY rating) AS row_num,
	title,
	rating,
	length 
FROM film;
...
SELECT 
	...
	PERCENT_RANK() OVER (
		ORDER BY `length`
	) AS length_percent_rank, -- less than
	CUME_DIST () OVER (
		ORDER BY `length`
	) AS length_cume_dist -- less than or equal
FROM film 
WHERE rating = 'G';
